...
# 首次训练
    ...
    rnn2 = torch.load(second_stock_code + "_model.pkl")
    # 重新初始化学习率
    optimizer1 = torch.optim.Adam(rnn1.parameters(), lr=LR)
    optimizer2 = torch.optim.Adam(rnn2.parameters(), lr=LR)

    scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer1, step_size=10, gamma=0.1, last_epoch=-1)
    scheduler2 = torch.optim.lr_scheduler.StepLR(optimizer2, step_size=10, gamma=0.1, last_epoch=-1)
    # 开始训练
    for step in range(EPOCH):
        for tx, ty in train_loader:
            ...
            output2 = rnn2(tx)
            # 计算损失
            loss1 = loss_func(torch.squeeze(output1[:, -1, :]), torch.squeeze(ty))
            loss2 = loss_func2(torch.squeeze(output2[:, -1, :]), torch.squeeze(ty))
            # 梯度清零
        ...
        scheduler2.step()
        # 打印每个epoch的损失
        print(f"模型一--三次训练-第 {step + 1} 个 Epoch 的损失: {loss1.item()}")
        print(f"模型二--三次训练-第 {step + 1} 个 Epoch 的损失: {loss2.item()}")

    torch.save(rnn1,first_stock_code+".pkl")
    torch.save(rnn2,second_stock_code+".pkl")
